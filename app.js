var express = require('express');
var path = require('path');
var favicon = require('serve-favicon');
var logger = require('morgan');
var cookieParser = require('cookie-parser');
var bodyParser = require('body-parser');
var cors = require('cors')

 // rutas
var index = require('./routes/index');
var users = require('./routes/users');
var usuarioRoute = require('./routes/usuario.route');
var equipoRoute = require('./routes/equipo.route');
var canalRoute = require('./routes/canal.route');
var chatRoute = require('./routes/chat.route');
var comentarioRoute = require('./routes/comentario.route');
var mensajeRoute = require('./routes/mensaje.route');
var perfilCanalRoute = require('./routes/perfilCanal.route');
var publicacionRoute = require('./routes/publicacion.route');

var app = express();

// view engine setup
app.set('views', path.join(__dirname, 'views'));
app.set('view engine', 'ejs');

// uncomment after placing your favicon in /public
// app.use(favicon(path.join(__dirname, 'public', 'favicon.ico')));
app.use(logger('dev'));
app.use(bodyParser.json());
// application/x-www-form-urlencoded
app.use(bodyParser.urlencoded({ extended: true }));
app.use(cookieParser());
app.use(express.static(path.join(__dirname, 'public')));
app.use('/knockout', express.static(path.join(__dirname, 'node_modules/knockout/build')));
app.use('/jquery', express.static(path.join(__dirname, 'node_modules/jquery/dist')));
app.use(cors({origin: '*'}))

app.use('/', index);
app.use('/users', users);
app.use('/', usuarioRoute);
app.use('/', canalRoute);
app.use('/', equipoRoute);
app.use('/', chatRoute);
app.use('/', mensajeRoute);
app.use('/', publicacionRoute);
app.use('/', comentarioRoute);
app.use('/', perfilCanalRoute);

// catch 404 and forward to error handler
app.use(function(req, res, next) {
  var err = new Error('Not Found');
  err.status = 404;
  next(err);
});

// error handler
app.use(function(err, req, res, next) {
  // set locals, only providing error in development
  res.locals.message = err.message;
  res.locals.error = req.app.get('env') === 'development' ? err : {};

  // render the error page
  res.status(err.status || 500);
  res.render('error');
});

module.exports = app;
